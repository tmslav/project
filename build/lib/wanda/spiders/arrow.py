...
class ArrowSpider(scrapy.Spider):
    ...
    def parse_1(self, response):
        ...
        for link in links:
            yield Request(urljoin(self.base_url, link), self.parse_2)
    ...
    def parse_2(self, response):
        ...
        if link and link[0] != 'Register':
            yield Request(url_pattern.format(1, link[0]), self.parse_3)
    ...
    def parse_3(self, response):
        ...
        if pd['data'].get('totalResultCount'):
            ...
            for i in range(1, number_of_products + 1):
                yield Request(url_pattern.format(i), self.parse_4, dont_filter=True)
    ...
    def parse_4(self, response):
        ...
        for result in results:
            yield Request(urljoin(self.base_url, result['partUrl']), self.parse_item, meta={'pd': result})
    ...
    def parse_item(self, response):
        ...
        return Request(url, self.parse_price, meta={'i': i})
...
#            #yield pi
        ...
        if self.processed_items == self.drop_count_items:
            raise CloseSpider("Sample scraped")
